...
for i = 1:length(m)
    for j = 1:length(Ns)
    H = 600*0.95; %Potencial total do motor

red = 5; %Redução geral do sistema
Na = Ns*(red-1);
Ds(i,j) = Ns(j).*m(i);
Da(i,j) = Na(j).*m(i);
...
eng_p = 5; % N# de eng planetárias
He = H/5; %Potencia transmitida em cada engrenamento planeta-solar
...
[col] = find (Shp(1,:)>1 & Shp(1,:)<1.3, 100);
for k = 1:length(col)
    coluna = col(k);
    SHG = Shp(1,coluna);
    WT = Wt(1,coluna);
